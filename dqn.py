...
def calc_rewards(prev_states, states):
  return [calc_reward(prev_states[i], states[i]) for i in range(len(prev_states))]

class DQN:
  def __init__(self, gamma=0.98, learning_rate=0.00001, update_targer_every=200, device=torch.device("cpu")):
    self.device = device
  ...
  def update(self, batch):
    prev_states, actions, states, _, dones = batch
    ...
    pred_Q_s_a = self.model(prev_states).gather(1, actions.unsqueeze(1)).squeeze(1)
...
class EpsilonGreedyAgent:
  def __init__(self, inner_agent, epsilon_max = 0.5, epsilon_min = 0.1, decay_steps=20000):
  ...
  def act(self, state):
    ...
    epsilon = self.epsilon_max + (self.epsilon_min - self.epsilon_max) * self.steps / self.decay_steps
    if epsilon > np.random.random():
      return np.random.randint(0, 3)
    else:
      return self.inner_agent.act(state)

def play_episode(env, agent:Agent):
  state = env.reset()
  done = False
  trajectory = []
  total_reward = 0
...
def train_agent(env, exploration_agent, exploitation_agent, updater, env_steps=20000, exploit_every=20000):
  steps_count = 0
  exploits = 0
  rewards = []
  steps = []
...
rewards, steps = train_agent(env, epsilon_greedy, dqn_agent, updater, env_steps=max_steps, exploit_every=max_steps//steps_per_episode)